...
const HiveController = {
    ...
    create: async function (req, res) {
        try {
            ...
            const longitude = Number(location_x);
            const latitude = Number(location_y);

            const precipitation = req.body.precipitation ?? req.body.precipitiation ?? 0;
            ...
            const result = await weather.insertOrUpdate();

            return res.status(200).json({
                message: "Weather saved (inserted or updated)",
                result
            });

        } catch (err) {
            console.log(err);
            return res.status(500).send("Napaka pri ustvarjanju vremena");
        }
    },
    ...
    list: function (req, res) {
        WeatherModel.getAllLatestPerLocation()
...
//     }
    ...
    remove: function (req, res) {
        ...
        WeatherModel.deleteById(hiveId, userId)
            .then(() => {
                return res.status(200).send("Uspesno zbrisan panj")
            })
            .catch(err => {
                console.error(err);
